...
register_page(__name__, name="piRNA Classification", path="/pirna")

start_button_pirna = dmc.Grid(
    children=[
        dmc.GridCol(
        dmc.Button(
            id="start-button-pirna",
...
fetch_example_pirna = dmc.Button(
    id="fetch-example-pirna",
    children="Use Example",
    leftSection=DashIconify(icon="line-md:compass-twotone-loop"),
)

pirna_fasta_input = dmc.Grid(
    children=[
        dmc.GridCol(fasta_textarea, span="10"),
        dmc.GridCol(fetch_example_pirna, span="2"),
    ],
    align="flex-end",
)
...
@callback(
    ...
    Input("fetch-example-pirna", "n_clicks"),
    prevent_initial_call=True,
)
def fetch_example_pirna(n_clicks):
    fasta_text = ""
    for record in SeqIO.parse(example_fastas["pirna"], "fasta"):
...
clientside_callback(
    ...
    """,
    Output("start-button-pirna", "loading", allow_duplicate=True),
    Input("start-button-pirna", "n_clicks"),
    prevent_initial_call=True,
)
...
@callback(
    ...
    Input("start-button-pirna", "loading"),
    State("fasta-text", "value"),
    prevent_initial_call=True,
)
def start_infer_pirna(loading: bool, fasta_text: str):
    if not fasta_text:
    ...
    if loading:
        try:
            ...
            return False, ret.to_dict("records"), [{"field": i} for i in ret.columns], {"fileName": f"pirna_results_{now}.csv"}, False, success_alert
        except Exception as e:
            return False, [], [], None, True, [fail_alert, f"Error: {e}"]

layout = [
    html.Div(children="piRNA Classification", style={"textAlign": "center", "fontSize": 30}),
    html.Hr(),
    dmc.MantineProvider(
