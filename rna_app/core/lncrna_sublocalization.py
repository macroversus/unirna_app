...
def lazy_inference_runner(opts: LazyInferenceOptions) -> int:
    ...
    infer = LazyInferencer(
        ...
        checkpoint="/tmps/lncrna_sublocalization/best_roc_796.pth",
        batch_size=1,
    ...
    data_list = pd.DataFrame({"cdna": sequence, "tag": 0})
    result_unirna = infer.run(data_list)

    result_label = [num for num in result_unirna["tag"]]
    result_df = pd.DataFrame({"lncrna": sequence, "tag": result_label})
