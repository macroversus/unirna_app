...
def parse_args():
    ...
    parser.add_argument('--elitism_ratio', type=float, default=0.2,
                        help='Fraction of top sequences to retain from previous generation (精英保留比例)')
...
class MaskedGenerator:
    ...
    def __init__(
        ...
        selection_mode: str = "hard",  # "hard" or "soft"
    ...
    def mutate_single(
        ...
        # 计算实际突变位点数量，至少突变2个
        num_mutate = max(2, int(len(mutable_positions) * mutation_rate))
...
def optimize_sequences(
    ...
    save_sequences([generator.template.upper()], ['template'], "Initial", output_csv)
...
def main():
    ...
    # 初始化序列生成器
    generator = MaskedGenerator(args.template, allow_mutate_all_sites=args.allow_mutate_all_sites, protected_regions=args.protected_regions, device=device)
