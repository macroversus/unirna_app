...
class LazyInferenceOptions(BaseModel):
    ...
    input_data: InputFilePath = Field(default=None, description="fasta file")
...
def lazy_inference_runner(opts: LazyInferenceOptions) -> int:
    ...
    infer = LazyInferencer(
        ...
        checkpoint = "/tmps/test/r2_095.pth",
        batch_size=1,
    ...
    data_list = pd.DataFrame({'utr':sequence,'scaled_rl':0})
    result_unirna = infer.run(data_list)
    
    result_label = [item for lst in result_unirna['scaled_rl'] for item in lst]
    result_df = pd.DataFrame({'utr_sequence': sequence, 'scaled_rl':result_label})
